    ...
    <main>
      <div class="content">
        <div class="l">
          ...
          <ul>
            <li
              ...
              :key="index"
              @mouseenter="imgTab(img.src, index)"
            >
              <img
                ...
                height="100"
                alt=""
                :style="{ opacity: currentImgInx === index ? 1 : 0.5 }"
              />
            </li>
          </ul>
        </div>
        <div class="r">
          ...
          <div class="down">
            <section
              v-for="(paramet, pidx) in productInfo.parameterJson"
              :key="pidx"
            >
              <strong>{{ paramet.title }}</strong>
              <ul>
                <li
                  :class="{ active: item.currentActivate }"
                  v-for="(item, pdx) in paramet.parameters"
                  :key="pdx"
            ...
            <div style="margin: 10px 0">
              <strong>数量 <span>*礼品库存{{productInfo.stock}}</span></strong>
              <div class="step">
      ...
      <aside>
        <h3>你还可以兑换</h3>
        <ul>
          <li v-for="(imgs, pdx) in themYouCanBuy" :key="pdx">
            <div class="l">
              <img :src="'https://sc.wolfcode.cn' + imgs.img" alt="" />
            </div>
            <div class="r">
              <div class="title">{{ imgs.name }}</div>
              <div class="score">
                <span>{{ imgs.coin }}</span>
                积分
              </div>
...
<script>
import { getProductsDetail,getCarts } from "../request/HttpApi";
export default {
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    addToCart(){
      ...
      getCarts({
        ...
        if(res.code===0){
           this.$toast({
          message:'当前商品成功添加到购物车',
          type:'success'
        });
        // 更新页面数据
        this.currentImgInx=0;
        this.num =1;
        this.fetchData(this.$route.params.pid);
        // 更新当前购物车数量
