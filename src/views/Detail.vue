    ...
    <main>
      <div class="content">
        <div class="l">
          ...
          <ul>
            <li
              v-for="(img, index) in productInfo.imgAltas"
              :key="index"
              @mouseenter="currentImg(img.src, index)"
            >
              <!-- img里面的index是固定的,currentImgOpacity变化 -->
              <img
                :src="'https://sc.wolfcode.cn' + img.src"
                width="100"
                height="100"
                alt=""
                :style="{ opacity: currentImgOpacity == index ? 1 : 0.5 }"
              />
            </li>
          </ul>
        </div>
        <div class="r">
          ...
          <div class="down">
            <section
              v-for="(parameter, index) in productInfo.parameterJson"
              :key="index"
            >
              <strong>选择{{ parameter.title }}</strong>
              <ul>
                <li
                  :class="{ active: param.currentActivate }"
                  v-for="(param, pidx) in parameter.parameters"
                  :key="pidx"
                  @click="toDetailPid(param.id)"
                >
                  {{ param.title }}
                </li>
              </ul>
...
<script>
import { getProductsDetail, addShopCart } from "../request/httpAPI";
export default {
  components: {},
  data() {
    return {
      productInfo: {},
      themYouCanBuy: [],
      currentImgOpacity: 0, //当前图片的透明度
      num: 9, //商品数量
    };
  },
  methods: {
    //   展示图显示当前鼠标移入图片,当前图片的透明度变化
    currentImg(src, idx) {
      this.productInfo.coverImg = src;
      this.currentImgOpacity = idx;
    },
    // 跳转到对应的商品
    toDetailPid(pid) {
      this.$router.push("/detail/" + pid);
      this.currentImgOpacity = 0; //重新跳转页面初始化图片透明度
      this.num = 1; //商品数量初始化
      // 再次调用商品详情接口
      this.fetchProductsDetail(pid);
    },
    // 商品详情
    fetchProductsDetail(pid) {
      getProductsDetail(pid).then((res) => {
        if (res.code === 0) {
          // console.log(res.data);
          this.productInfo = res.data.productInfo;
          // 商品介绍里面的图片需要添加域名才能正常访问
    ...
    stepFn(num) {
      ...
      } else if (this.num === this.productInfo.stock && num == 1) {
        return this.$toast({
          message: "数量大于库存",
          type: "error",
        });
    ...
    addToCart() {
      ...
      addShopCart({
        ...
        if (res.code === 0) {
          ...
          this.num = 1; //商品数量初始化
          // 再次调用商品详情接口
          this.fetchProductsDetail(this.$route.params.pid);
